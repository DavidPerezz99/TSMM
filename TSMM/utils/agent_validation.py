...
def _simulate_day(day_df: pd.DataFrame, trading_cfg: Dict[str, Any]) -> Dict[str, Any]:
    """Deterministic per-day simulation on minute candles.

    Signal proxy:
    - If `y_diff` exists: direction = sign(y_diff)
    - Else direction = sign(CLOSE.diff())
    """
    ...
    close = pd.to_numeric(df["CLOSE"], errors="coerce").fillna(method="ffill").fillna(method="bfill").values
