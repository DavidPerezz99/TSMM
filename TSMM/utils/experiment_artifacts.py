...
def best_r2_model(evaluation: Dict[str, Any]) -> Tuple[Optional[str], Optional[float]]:
    ...
    for model_name, payload in (evaluation or {}).items():
        ...
        if best_score is None or score > best_score:
            best_name, best_score = str(model_name), score
    ...
    *, models: Dict[str, Any], evaluation: Dict[str, Any],
    ...
    try:
        ...
        saved_path = save_best_model(
            selected_models, selected_evaluation, str(model_dir), logger, config=config
        )
        if not saved_path or not any(path.is_file() for path in model_dir.rglob("*")):
        ...
        (staging / "evaluation.json").write_text(
            json.dumps(_jsonify(selected_evaluation), indent=2, allow_nan=False), encoding="utf-8"
        )
        (staging / "forecasts.json").write_text(
            json.dumps(_jsonify({model_name: (future_forecasts or {}).get(model_name)}), indent=2, allow_nan=False),
            encoding="utf-8",
        )
        ...
        data_manifest: Dict[str, Any] = {
            ...
            "file_fingerprint": _sampled_file_fingerprint(data_path),
        ...
        (staging / "data_manifest.json").write_text(
            json.dumps(_jsonify(data_manifest), indent=2, allow_nan=False), encoding="utf-8"
        )
        checksums = {
        ...
        (staging / "manifest.json").write_text(
            json.dumps(_jsonify(manifest), indent=2, allow_nan=False), encoding="utf-8"
        )
